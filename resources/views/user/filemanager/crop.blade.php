<div class="row fill">
    ...
    <div class="col-md-4 fill">
        <div class="text-center">
            ...
            <br>
            <button class="btn btn-primary" onclick="performCrop()">{{ Lang::get('lfm.btn-crop') }}</button>
            <button class="btn btn-info"
                    onclick="loadItems()">{{ Lang::get('lfm.btn-cancel') }}</button>
            <form action="{{ route('customer.file.getCrop') }}" role='form' name='cropForm' id='cropForm' mathod='post'>
...
<script>
    ...
    function performCrop() {
        $.ajax({
            type: "GET",
            dataType: "text",
            url: "{{ route('customer.file.getCropimage') }}",
            data: {
                img: '{{ $img }}',
