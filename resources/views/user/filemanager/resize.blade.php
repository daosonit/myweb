    ...
    <div class="row fill">
        ...
        <div class="col-md-4 fill">
            ...
            <table class="table table-compact table-striped">
                ...
                @if ($scaled)
                    <tr>
                        <td>{{ Lang::get('resize-ratio') }}</td>
                        <td>{{ number_format($ratio, 2) }}</td>
                    </tr>
                    <tr>
                        <td>{{ Lang::get('resize-scaled') }}</td>
                        <td>
                            {{ Lang::get('resize-true') }}
                        </td>
                    </tr>
                @endif
                <tr>
                    <td>{{ Lang::get('resize-old-height') }}</td>
                    <td>{{ $original_height }}px</td>
                </tr>
                <tr>
                    <td>{{ Lang::get('resize-old-width') }}</td>
                    <td>{{ $original_width }}px</td>
                </tr>
                <tr>
                    <td>{{ Lang::get('resize-new-height') }}</td>
                    <td><span id="height_display"></span></td>
                </tr>
                <tr>
                    <td>{{ Lang::get('resize-new-width') }}</td>
                    <td><span id="width_display"></span></td>
                </tr>
                </tbody>
            </table>

            <button class="btn btn-primary" onclick="doResize()">{{ Lang::get('btn-resize') }}</button>
            <button class="btn btn-info" onclick="loadItems()">{{ Lang::get('btn-cancel') }}</button>
...
<script>
    ...
    function doResize() {
        $.ajax({
            type: "GET",
            dataType: "text",
            url: "{{ route('customer.file.performResize') }}",
            data: {
                img: '{{ $img }}',
