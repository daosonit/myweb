<script>
    ...
    $(document).ready(function () {
        bootbox.setDefaults({locale: "{{ Lang::get('lfm.locale-bootbox') }}"});
        // load folders
        loadFolders();
        loadItems();
        setOpenFolders();
    ...
    $('#add-folder').click(function () {
        bootbox.prompt("{{ Lang::get('lfm.message-name') }}", function (result) {
            if (result !== null) {
                createFolder(result);
            }
        });
    ...
    $('#upload-btn').click(function () {
        ...
        function showRequest(formData, jqForm, options) {
            $('#upload-btn').html('<i class="fa fa-refresh fa-spin"></i> {{ Lang::get("lfm.btn-uploading") }}');
            return true;
        }

        function showResponse(responseText, statusText, xhr, $form) {
            $('#uploadModal').modal('hide');
            $('#upload-btn').html('{{ Lang::get("lfm.btn-upload") }}');
            if (responseText != 'OK') {
                notify(responseText);
            }
            $('#upload').val('');
        ...
        function showError(jqXHR, textStatus, errorThrown) {
            $('#upload-btn').html('{{ Lang::get("lfm.btn-upload") }}');
            if (jqXHR.status == 413) {
                notify('{{ Lang::get("lfm.error-too-large") }}');
            } else if (textStatus == 'error') {
                notify('{{ Lang::get("lfm.error-other") }}' + errorThrown);
            } else {
                notify('{{ Lang::get("lfm.error-other") }}' + textStatus + '<br>' + errorThrown);
            }
        }
    ...
    function loadFolders() {
        $.ajax({
            type: 'GET',
            dataType: 'html',
            url: '{{ route("customer.file.getFolders") }}',
            data: {
    ...
    function loadItems() {
        ...
        $.ajax({
            type: 'GET',
            dataType: 'html',
            url: '{{ route("customer.file.getItems") }}',
            data: {
    ...
    function createFolder(folder_name) {
        $.ajax({
            type: 'GET',
            dataType: 'text',
            url: '{{ route("customer.file.getAddfolder") }}',
            data: {
                name: folder_name,
                working_dir: $('#working_dir').val(),
                type: $('#type').val()
    ...
    function rename(item_name) {
        bootbox.prompt({
            title: "{{ Lang::get('lfm.message-rename') }}",
            value: item_name,
            callback: function (result) {
                if (result !== null) {
                    $.ajax({
                        type: 'GET',
                        dataType: 'text',
                        url: '{{ route("customer.file.getRename") }}',
                        data: {
                            file: item_name,
                            working_dir: $('#working_dir').val(),
                            new_name: result,
    ...
    function trash(item_name) {
        ...
        bootbox.confirm("{{ Lang::get('lfm.message-delete') }}", function (result) {
            if (result == true) {
                $.ajax({
                    type: 'GET',
                    dataType: 'text',
                    url: '{{ route("customer.file.getDelete") }}',
                    data: {
                        working_dir: $('#working_dir').val(),
                        items: item_name,
                        type: $('#type').val()
    ...
    function cropImage(image_name) {
        $.ajax({
            type: 'GET',
            dataType: 'text',
            url: '{{ route("customer.file.getCrop") }}',
            data: {
                img: image_name,
                working_dir: $('#working_dir').val(),
                type: $('#type').val()
    ...
    function resizeImage(image_name) {
        $.ajax({
            type: 'GET',
            dataType: 'text',
            url: '{{ route("customer.file.getResize") }}',
            data: {
                img: image_name,
                working_dir: $('#working_dir').val(),
                type: $('#type').val()
    ...
    function download(file_name) {
        location.href = '{{ route("customer.file.getDownload") }}?' + 'working_dir=' + $('#working_dir').val() + '&type=' + $('#type').val() + '&file=' + file_name;
    }
